...
class MulticoreTSNE:
    ...
    * method
    ...
    __version__ = "MODIFIED_WITH_EARLY_STOP"
    ...
    def __init__(
        ...
        ), "init must be 'random' or array"
        if isinstance(init, np.ndarray):
            assert init.ndim == 2, "init array must be 2D"
            assert (
                init.shape[1] == n_components
            ), "init array must be of shape (n_instances, n_components)"
            self.init = np.ascontiguousarray(init, float)

        self.ffi = cffi.FFI()
        self.ffi.cdef(
            """
            void tsne_run_double(
    ...
    def fit_transform(self, X, _y=None):
        ...
        if init_from_Y:
            ...
            assert (
                X.shape[0] == Y.shape[0]
            ), "n_instances in init array and X must match"
        else:
        ...
        error_per_point = np.zeros((N,), dtype=float)
        cffi_error_per_point = self.ffi.cast("double*", error_per_point.ctypes.data)
        ...
        self.error_per_point_ = error_per_point
        return Y
